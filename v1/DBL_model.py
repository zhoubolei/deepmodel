...
class DBL_model(object):
    def __init__(self,model=None,algo=None,basepath=[],preproc=[0,0]): 
        # 1. data 
        self.numclass = model.layers[-1].n_classes
        self.ishape = np.append(model.input_space.shape,1)
        self.preprocess(basepath,preproc[0],preproc[1])
    ...
    def preprocess(self,basepath,train_pre=0,flip=3):
        myDataset = LoadData(basepath)
        datasets = myDataset.loadTrain(preprocessFLAG=train_pre,flipFLAG=flip)
        self.ds_train = DataPylearn2(datasets[0],self.ishape,self.numclass)
        self.ds_valid = DataPylearn2(datasets[1],self.ishape,-1)
        self.ds_test = DataPylearn2(myDataset.loadTest(train_pre),self.ishape,-1)
    ...
    def test(self,ds2):
        ...
        y = np.concatenate(y)
        acc = 0
        if len(ds2.y)>0 and len(y)==len(ds2.y):
